...
local function vungleAdListener( event )
    if ( event.phase == "displayed" and not event.isError) then
        print("adStart: " .. tostring(event.placementID))
    end
    if ( event.phase == "displayed" and event.isError ) then
    ...
    if ( event.phase == "vungleSDKlog" ) then
...
pos = pos + eh + margin
main:insert( display.newText( { text = appID, x = _W/2, y = pos, font = native.systemFont, width = ew, height = eh, fontSize = fontSize, align = "center" } ) )
pos = pos + eh + margin

local function handleInit( event )
    print("Initializing...")
    if ( "ended" == event.phase ) then
        ads.init(appID, vungleAdListener)
    end
end
